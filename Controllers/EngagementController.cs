        ...
        public async Task<IActionResult> CreateReview(ReviewDto dto)
        {
            ...
            if (dto.PackageId.HasValue)
            {
                var hasCompletedReservation = await _context.Reservations
                    ...
                    .Include(r => r.Package)
                    .AnyAsync(r =>
                        r.PackageId == dto.PackageId.Value &&
                        r.Customer.Email == dto.UserEmail &&
                        r.Package.AgencyName == dto.AgencyName &&
                        r.IsConfirmed &&
                        !r.IsCancelled &&
                        r.DataKthimit.Date < DateTime.Now.Date);

                if (!hasCompletedReservation)
                    return BadRequest(new { message = "Review mund të lihet vetëm pasi udhëtimi i konfirmuar të jetë përfunduar." });
            }

            // Each customer has one active review for the same package/agency.
            // If the customer changes the review, the existing one is updated.
            // This keeps the average rating accurate.
