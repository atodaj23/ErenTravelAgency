...
function loadReservations() {
  ...
  fetch(`${API_BASE_URL}/reservations`)
    ...
    .catch((error) => {
      ...
      container.innerHTML =
        '<div class="col-12"><p class="text-center text-danger fs-5 mt-5">Ndodhi një gabim gjatë ngarkimit të rezervimeve.</p></div>';
    });
}
...
function renderReservations() {
  ...
  if (reservations.length === 0) {
    container.innerHTML =
      '<div class="col-12"><p class="text-center text-muted fs-5 mt-5">Nuk ka rezervime të bëra.</p></div>';
    return;
  }
  ...
  for (let i = 0; i < reservations.length; i++) {
    ...
    const cardHTML =
      ...
      (!res.confirmed
        ? '<button class="btn btn-success btn-sm" onclick="confirmReservation(' +
          res.id +
          ')">Konfirmo</button>'
        : "") +
  ...
  const totalHTML =
    ...
    '<div class="col-md-6">' +
    '<h4 class="mb-0">Shuma Totale: <span class="text-primary">' +
    totalPrice.toFixed(2) +
    "€</span></h4>" +
    '<p class="text-muted small mb-0">Total për ' +
    reservations.length +
    " paketa</p>" +
    "</div>" +
...
function confirmAllReservations() {
  ...
  if (
    !confirm("A jeni të sigurt që doni të konfirmoni të gjitha rezervimet?")
  ) {
    return;
  }
